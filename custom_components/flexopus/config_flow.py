...
AUTH_SCHEMA = vol.Schema(
    {vol.Required(CONF_ACCESS_TOKEN): cv.string, vol.Optional(CONF_TENANT_URL): cv.string}
)
...
class FlexopusConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                await validate_auth(user_input[CONF_ACCESS_TOKEN], user_input[CONF_TENANT_URL])
            except ValueError:
                errors["base"] = "auth"
            if not errors:
